...
namespace Bin
{
	...
	public class Conversion
	{
        ...
        public static byte[] desmembraInt16(int valor)
        {
            byte[] baArray = new byte[2];

            baArray[1] = (byte)(valor & 0x000000FF);
            valor = valor >> 8;
            baArray[0] = (byte)(valor & 0x000000FF);

            return baArray;
        }

        public static byte[] desmembraInt32(int valor)
        {
            byte[] baArray = new byte[4];
            baArray = System.BitConverter.GetBytes(valor);
            Array.Reverse(baArray);
            return baArray;
        }

        public static byte[] desmembraFloat32(float valor)
        {
            byte[] baArray = new byte[4];
            baArray = System.BitConverter.GetBytes(valor);
            Array.Reverse(baArray);
        ...
        public static float montaFloat32(byte[] valorArray)
        {
            float valor = 0.0f;
            Array.Reverse(valorArray);
            valor = System.BitConverter.ToSingle(valorArray, 0);
            return valor;
        }   
